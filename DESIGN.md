...
The deployed palette is sourced from `themes/current-theme.json`. Alternative palettes live in the development-only Design lab and may be previewed across routes and reloads without changing that production source. `npm run theme:list` lists the catalog; `npm run theme:apply:<theme-id>` deliberately promotes one palette to the deployed theme and synchronizes static fallback surfaces.

## Typography

Mineral uses Source Sans 3 as its production baseline. Design lab alternatives intentionally explore different complete type systems—grotesk, condensed poster, serif editorial, slab, and monospace—alongside their color identities. Promoting a theme updates the typography recorded in this frontmatter. Body copy stays between 60 and 75 characters in every direction.
